...
local commandOptions = {
	...
	allowooc =
	"Allow out of combat targets to be multicursed.  Would only consider using this solo to avoid potentially griefing raids/dungeons by pulling unintended mobs.",
...
local commands = {
	["curse"] =
	"/cursive curse <spellName:str>|<guid?:str>|<options?:List<str>>: Casts spell if not already on target/guid",
	["multicurse"] =
	"/cursive multicurse <spellName:str>|<priority?:str>|<options?:List<str>>: Picks target based on priority and casts spell if not already on target",
}
...
function IsUnitCrowdControlled(guid)
...
local function pickTarget(selectedPriority, spellNameNoRank, checkRange, options, customFilter)
		...
		if shouldDisplay then
			...
			if ignoreInFight or Cursive.filter.infight(guid) or guid == currentTargetGuid then
				...
				if checkRange == false or CheckInteractDistance(guid, 4) then
					...
					if not Cursive.curses:HasCurse(spellNameNoRank, guid, refreshTime) and not IsUnitCrowdControlled(guid) then
						local mobHp = UnitHealth(guid)
						if not minHp or mobHp >= minHp then
							if not customFilter or customFilter(guid) then
								local primaryValue
								local secondaryValue = -1
								if selectedPriority == "HIGHEST_HP" then
									primaryValue = UnitHealth(guid)
								elseif selectedPriority == "LOWEST_HP" then
									primaryValue = -UnitHealth(guid)
...
function Cursive:Curse(spellName, targetedGuid, options, customFilter)
	...
	if targetedGuid and not Cursive.curses:HasCurse(spellNameNoRank, targetedGuid, options["refreshtime"]) and not IsUnitCrowdControlled(targetedGuid) and (not customFilter or customFilter(targetedGuid)) then
...
function Cursive:Multicurse(spellName, priority, options, customFilter)
